...
def load_and_preprocess():
    ...
    if not os.path.exists('credit_card_fraud_dataset.csv'):
        raise FileNotFoundError("credit_card_fraud_dataset.csv not found in current directory")
    ...
    df['TimeCategory'] = df['TimeCategory'].fillna('Afternoon')
...
def preprocess_data(df):
    ...
    # Convert to DataFrames - handle both sparse and dense cases
    def to_dataframe(matrix, columns):
    ...
    X_train_df = to_dataframe(X_train_smote, all_feature_names)
    X_test_df = to_dataframe(X_test_preprocessed, all_feature_names)
    y_train_df = pd.DataFrame(y_train_smote, columns=['IsFraud'])
...
if __name__ == "__main__":
    ...
    print("- data/X_train.csv (Training features)")
    print("- data/X_test.csv (Test features)")
    print("- data/y_train.csv (Training labels)")
    print("- data/y_test.csv (Test labels)")
    print("- models/preprocessor.joblib (Preprocessor object)")